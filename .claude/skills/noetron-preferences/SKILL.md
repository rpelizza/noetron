...
The global behavior floor beneath every task that produces content. It ships with the harness; the project's own record (`noetron/setup/preferences.md`) captures what this user decided — and **project preferences prevail over this floor** wherever they conflict. A lasting preference stated mid-task gets recorded there (propose-confirm), never just obeyed once and forgotten.
...
- User-facing text (UI, errors, CLI output) is written in the product's language (`noetron/setup/preferences.md`); error messages are actionable — what failed and what to do about it.
...
2. The project's recorded preferences (`noetron/setup/preferences.md`).
