...
1. Start from what already summarizes: the `.noetron/` workspace — `state.md`, `profile.md`, `domain-skills.md`, `learnings.md`, and **`history/INDEX.md`, never the `history/` directory**: scan the index, open only the entries whose slug or signal touches this territory. Reading every past entry costs more on every task as the project ages, and answers almost none of them. Then domain skills (the ones covering the territory first), manifests, READMEs. Then code and tests.
...
- `noetron-plan` — its EXPLORE step, and every claim it makes about current behavior, run here.
- `noetron-router` — the `explore` node of every chain, and the evidence a contested tier needs.
- `noetron-branch` — the node the graph sends this one to in the `standard` and `large` chains.
  Exploring is the only node that runs before isolation, because it is the only one that writes
  nothing; the ground is cut once the facts are in.
